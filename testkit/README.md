...
`fixture_test.go` 锁定 `Config("fixture")` 的字段和 `Validate` 结果，并验证 `RequireNoError(t, nil)` 可用。`golden_test.go` 锁定 golden 断言的匹配路径。`observability_test.go` 锁定 recording logger、metrics 和 tracer 的基本行为。生成后的基础库需要保留这组最小测试，以防测试夹具随包名替换、配置 contract、可观测性 contract 或稳定输出漂移。
