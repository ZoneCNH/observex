...
- `CHECK_STATUS=passed VERSION=v0.3.5 GOWORK=off make evidence`
- `RELEASE_EVIDENCE_REQUIRE_PASSED=1 VERSION=v0.3.5 GOWORK=off make release-evidence-check`
...
`release-final-check` 要求所有 gate 状态为 `passed`，并要求 git 工作区为 `clean`。如果只是开发中自测，`VERSION=v0.3.5 make release-check` 已足够；它允许工作区显示 `dirty`，但仍会验证 manifest 和当前源码内容一致。
