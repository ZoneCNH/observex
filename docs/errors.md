...
| `ErrorKindCanceled` | `canceled` | context canceled 或调用方主动取消。 | 否 |
| `ErrorKindNotFound` | `not_found` | 资源不存在。 | 否 |
| `ErrorKindAlreadyExists` | `already_exists` | 创建目标已存在。 | 否 |
...
- `MapError` 必须把 `context.Canceled` 映射为 `canceled`，把 `context.DeadlineExceeded` 映射为 `timeout`。
- `ErrorKind`、`NewError`、`WrapError` 和 `IsKind` 与 `foundationx` 保持兼容，避免派生基础库重复定义错误语义。
