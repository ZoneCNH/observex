...
- blocker 标记：`external_downstream_unavailable`。
...
| Failure/blocker | 若失败，记录精确错误和下一步；若不可用，使用 `external_downstream_unavailable` |
...
2. 当真实下游不可用时，release evidence 必须保留 `external_downstream_unavailable` 并把结论标记为 non-final blocker。
