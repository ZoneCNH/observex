...
- `GOWORK=off VERSION=v0.3.4 make release-check` 通过，并以 `CHECK_STATUS=passed` 生成未提交的 `release/manifest/v0.3.4.json` Evidence artifact。
...
- 不作为 xlib-standard 模板渲染目标（observex 是 concrete library）。

## 可追踪性

- 目标：`GOAL-20260601-001`
- 模块：`github.com/ZoneCNH/observex`
- 标准基线：`xlib-standard` `v1.0.0`
- 共享基础依赖：`github.com/ZoneCNH/foundationx`（迁移中 → v0.4 截止）
