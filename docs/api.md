...
- `observex`：生成的仓库名称。
- `github.com/ZoneCNH/observex`：生成的 Go module 路径。
- `observex`：生成的包名。
...
使用 `scripts/render_template.sh` 生成具体基础库时，公共包目录会从 `pkg/observex` 移动到 `pkg/observex`，代码 imports、文档占位符和 module path 会同步替换。
