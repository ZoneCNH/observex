...
func TestConfigValidateRejectsNegativeTimeout(t *testing.T) {
	err := Config{Name: "observex", Timeout: -time.Second}.Validate()
	if err == nil {
...
func TestConfigSanitizeMasksSecret(t *testing.T) {
	sanitized := Config{Name: "observex", Timeout: time.Second, Secret: "plain-text"}.Sanitize()
	if sanitized.Secret != "***" {
		t.Fatalf("expected masked secret, got %q", sanitized.Secret)
	}
	if sanitized.Name != "observex" {
