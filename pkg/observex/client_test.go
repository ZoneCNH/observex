...
func TestNewRejectsCanceledContext(t *testing.T) {
	...
	_, err := New(ctx, Config{Name: "observex"})
	if err == nil {
		t.Fatal("expected canceled context to fail")
	}
	if !IsKind(err, ErrorKindCanceled) {
		t.Fatalf("expected canceled error, got %T %[1]v", err)
	}
...
func TestCloseIsIdempotent(t *testing.T) {
	metrics := &recordingMetrics{}
	client, err := New(context.Background(), Config{Name: "observex"}, WithMetrics(metrics))
	if err != nil {
...
func TestCloseRejectsCanceledContext(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	client, err := New(context.Background(), Config{Name: "observex"})
	if err != nil {
		t.Fatalf("new client: %v", err)
	}
